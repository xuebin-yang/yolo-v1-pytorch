...
def voc_eval(preds, target, VOC_CLASSES=VOC_CLASSES, threshold=0.5, use_07_metric=True,):
    ...
    for i, class_ in enumerate(VOC_CLASSES):
        ...
        if len(pred) == 0:  # 如果这个类别一个都没有检测到的异常情况
            ...
            break
        # print(pred)
        image_ids = [x[0] for x in pred]   # 根据预测统计该类别的所有图像
        confidence = np.array([float(x[1]) for x in pred]) # 根据预测统计该类别的所有置信度
        BB = np.array([x[2:] for x in pred])  # 根据预测统计该类别的所有bbox
        # sort by confidence
        sorted_ind = np.argsort(-confidence)  # argsort(x)函数是将x中的元素从小到大排列，提取其对应的index(索引)，然后输出到y,这里有负号，所以是从大到小排列，返回索引
        sorted_scores = np.sort(-confidence)  # 将置信度从大到小排列，正好匹配sorted_ind
        BB = BB[sorted_ind, :]   # 按照置信度重排 BB
        image_ids = [image_ids[x] for x in sorted_ind]   # image_ids按照置信度重排
...
if __name__ == '__main__':
    ...
    for index, image_file in enumerate(file_list):
        ...
        num_obj = (len(image_file) - 1) // 5   # 这个图片中有几个 obj
    ...
    model = resnet50()   # 这里的resnet50是变体，resnet_yolo结构
